...
**Draft-stage variant (arm E, 2026-09-06; design `Agentic_Knowledge_Base/docs/analogy_draft_injection_design.md`).** `analogy.draft=True` makes `draft_agent._inject_analogy_draft` call `engine.analogy.agent.retrieve_for_draft` once per run, for the FIRST draft only (virtual root has no child and none in flight; Phase 1 in `run.py` is sequential). Same corpus, tools, validation and budget; `mode="draft"` swaps the prompt (`SYSTEM_PROMPT_DRAFT`: abstract the TASK's structure — metric/label/evaluation relations — not a node's bottleneck; interventions are design commitments for a simple first solution) and the report wording (`REPORT_HEADING_DRAFT`). The packet (`build_task_packet`) is description + data preview + resource budget + offline pretrained models. The report is stored on the draft node's `analogy_report`, traced in `logs/analogy/draft_001.md`, and the index line carries `"stage": "draft"`. Drafts 2–3 and all later nodes get the arm-A prompt; with `analogy.improve=False` (arm E) `_inject_analogy` returns immediately.
...
- `verify_analogy_injection.py` — offline check of the analogy wiring: config YAML/dataclass agreement, BM25 on a toy corpus, report validation (citation rule, budget), the tools loop against a scripted client, injection into the improve prompt, the `analogy_report` journal field, and (section 7) the first-draft-only injection of arm E. No GPU, no API calls, ~2 s.
- `replay_analogy.py` — rebuild the packet for one node of an existing run from `journal.json` and run the agent against a corpus with the LLM in the environment. The fast loop for prompt/tokenizer changes (design doc §6.2); `--packet-only` needs no key. `--draft --desc description.md` (or `--draft --run <dir>`) replays the arm-E task-structure variant without any node.
