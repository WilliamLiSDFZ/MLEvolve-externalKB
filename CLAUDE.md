...
Outputs land in `runs/<timestamp>_<exp_name>/` with `logs/` (journal.json, filtered_journal.json, config.yaml, best_solution.py, injected_knowledge.md, kb_snapshot.json) and `workspace/` (input/, working/, submission/).

## Configuration

`config/config.yaml` is the single source of truth, loaded by `config/__init__.py:load_cfg` → merged with CLI args → validated against the `@dataclass Config` schema in `config/__init__.py`. The YAML holds the values, but the dataclass is **not** just type hints: `OmegaConf.merge` validates against it, so a top-level key present only in the YAML raises `ConfigKeyError` inside `load_cfg` and kills the run before it writes anything. Adding a config key means adding it in *both* places — `agent_paper_filter` shipped in the YAML alone and killed a job at startup. `python utils/verify_kb_injection.py` checks this (section 1d) without needing a GPU or API keys; run it after touching either file. The two model slots are `code` (generation) and `feedback` (parsing/review).
...
- **Agent paper filter** (`agent_paper_filter`, on by default) — in lazy mode, `_agent_filter_papers` drops unusable papers from title+abstract *before* any PDF is fetched, replacing the technique reranker that filtered only after paying for up to 20 full-text extractions. It never raises: any failure returns the candidate list unchanged, because a filter that can end a 12 h run is worse than no filter. **Its decision is cached on disk for correctness, not thrift** — `_chat` passes `temperature=0` only when `supports_sampling_params(model)`, and the reasoning models this project runs on are excluded, so the filter *samples*. The cache makes the decision consistent across arms; it does not make it reproducible, and two arms starting on a cold cache can still diverge. Warming it beforehand is what makes a draw safe.
- **`kb_snapshot.py`** — writes `logs/kb_snapshot.json` at cold start: the venues/years/counts in the retrieval pool, plus a digest. `injected_knowledge.md` records what a run *received*; this records what it *could have*, since `output/abstract_index/` is unversioned and `methodology_kb/` grows during normal operation. It captures state **before** the run's own extractions, so the end-of-run directory will legitimately hold more. A missing file means the run had no KB (arm A); a failed snapshot writes a file containing `"error"` instead.
...
**A/B/C experiments.** The `job-<task>-abc*.yaml` manifests launch matched arms of one comparison in a single file (base / kb / kb-variant), distinguished by `EXP_NAME` and `EXTRA_RUN_ARGS: "agent.seed=NN"`, with a distinct `SERVER_ID` per arm (grading port = 5005 + SERVER_ID). `k8s/prepare-task.sh` must run before a multi-arm launch: besides downloading data it *warms the retrieval caches*, which is a correctness requirement, not an optimization. Two LLM calls on the retrieval path — the query distiller and the paper filter — run **without** `temperature=0` (reasoning models reject sampling params), so on a cold cache each arm makes its own call, gets its own answer, and receives a different paper set. The script hard-gates on this and exits non-zero if the filter has no cached decision. In the first minutes of each run, confirm both `[Lazy] distilled query (cached)` and `[Filter] decision (cached ...)` say *cached*, not *new*, and that the B and C arms log the same injected-knowledge digest.
...
- `verify_kb_injection.py` — static check of where KB text actually reaches the prompts, plus the config-schema check above. No GPU, no API calls.
- `verify_filter_cache.py` — proves the paper-filter disk cache actually holds across arms. Its stub returns a *random* verdict on purpose: a deterministic stub would pass even if the cache did nothing, and check 1 is a negative control that must print `identical=False`.
...
- Default budgets are 500 steps / 12 h, enforced by the `timeout --kill-after` inside `run_single_task.sh`. Job manifests set `activeDeadlineSeconds: 86400` (24 h) as a backstop for what runs *outside* that timeout (entrypoint setup, the unbounded fusion step). It is measured from when the Job is **accepted**, so queue time counts against it — at the old 13 h a pod that queued for over an hour was killed mid-run, which silently hands paired arms different effective budgets. Don't lower it; check queue depth instead.
